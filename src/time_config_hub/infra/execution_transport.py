...
class RemoteTransport:
    ...
    def run(
        ...
        success = result["status_code"].name == "SUCCESS"
        return ExecutionResult(
    ...
    def put_file(self, local_path: str, remote_path: str) -> None:
        ...
        if result["status_code"].name != "SUCCESS":
            raise RuntimeError(
                f"[{self.target_label}] put_file failed: {result['error']}"
            )
    ...
    def get_file(self, remote_path: str, local_path: str) -> None:
        ...
        if result["status_code"].name != "SUCCESS":
            raise RuntimeError(
                f"[{self.target_label}] get_file failed: {result['error']}"
            )
