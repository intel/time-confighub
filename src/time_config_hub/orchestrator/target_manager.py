...
__all__ = ["assign_target_roles", "connect_remote_targets"]

logger = logging.getLogger("orchestrator.target_manager")


def assign_target_roles(config: PipelineConfig) -> list[Target]:
    """Return the list of targets with roles assigned based on topology.

    SINGLE_LOCAL → 1 target  (role=None)
    B2B          → 2 targets (1 reference + 1 mirror)
    MULTI_DUT    → N targets (1 reference + N-1 mirrors)

    :param PipelineConfig config: The orchestration configuration.
    ...
    if topo == DeploymentTopologyType.B2B:
        if len(targets) < 2:
            raise ValueError("B2B topology requires at least two targets (reference and mirror)")
        targets[0].role = "reference"
        targets[1].role = "mirror"
        return targets[:2]

    # MULTI_DUT — first target is reference, rest are mirrors
    if len(targets) < 2:
        raise ValueError("MULTI_DUT topology requires at least two targets (1 reference + N mirrors)")
    targets[0].role = "reference"
    for t in targets[1:]:
        t.role = "mirror"
    return targets
...
def connect_remote_targets(targets: list[Target], log: Callable[[str], None]) -> None:
